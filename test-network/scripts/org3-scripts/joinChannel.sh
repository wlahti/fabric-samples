...
setGlobals 3
setPeerAddressForCLI 3

OUTPUT="${CHANNEL_NAME}-config.block"
set -x
peer channel fetch config $BLOCK -o orderer.example.com:7050 --ordererTLSHostnameOverride orderer.example.com -c $CHANNEL_NAME --tls --cafile $ORDERER_CA >&log.txt
res=$?
{ set +x; } 2>/dev/null
cat log.txt
